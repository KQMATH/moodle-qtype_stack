 ...
 */
define(['jquery', 'qtype_stack/tex2max.amd', 'qtype_stack/visual-math-input'],
    function($, TeX2Max, VisualMath) {
        ...
        class wysiwyg {
            ...
            initialize() {
                ...
                for (let i = 0; i < this.inputs.length; i++) {
                    ...
                    let inputOptions = this.formatOptionsObj(
                        this.inputs[i].inputoptions);
                    ...
                    if (this.debug) {
                        let stackInputDebug = document.getElementById(
                            stackInputID + '_debug');
                        $stackInputDebug = $(stackInputDebug);

                        let latexInputDebug = document.getElementById(
                            latexInputID + '_debug');
                    ...
                    if (!wysiwyginput.$input.prop('readonly')) {
                        wysiwyginput.onEdit = ($input, field) => {
                            ...
                            $input.get(0).dispatchEvent(new Event('input')); // Event
                                                                             // firing
                                                                             // needs
                                                                             // to
                                                                             // be
                ...
                if (!readOnly) {
                    this.buildInputControls(
                        this.editorOptions['mathinputmode']);
                }
                else {
                    let selectionButton = $(
                        '#' + this.questionid + 'editor_selection');
            ...
            convert(latex, stackInputID) {
                ...
                if (!latex) {
                    ...
                    let stackValidationFeedback = document.getElementById(
                        stackInputID + '_val');
            ...
            hideTeX2MaXFeedback(stackInputID) {
                let existingFeedback = document.getElementById(
                    stackInputID + '_tex2max');
                ...
                if (stackValidationFeedback.style.display !== '') {
                    ...
                    parent.waitingTimer = setTimeout(() => {
                        parent.removeAllValidationClasses(
                            stackInputID + '_tex2max');
            ...
            renderTeX2MaXFeedback(errorMessage, stackInputID) {
                ...
                if (existingFeedback) {
                    ...
                    let existingErrorMessageParagraph = document.getElementById(
                        stackInputID + '_errormessage');
                    existingErrorMessageParagraph.innerHTML = errorMessage;

                }
                else {
                    let feedbackWrapper = document.createElement('div');
                    feedbackWrapper.setAttribute('class',
                        'tex2max-feedback-wrapper');
                    feedbackWrapper.setAttribute('id',
                        stackInputID + '_tex2max');

                    let feedbackMessageParagraph = document.createElement('p');
                    let errorMessageParagraph = document.createElement('p');
                    errorMessageParagraph.setAttribute('id',
                        stackInputID + '_errormessage');
            ...
            showOrHideCheckButton() {
                for (let i = 0; i < this.inputs.length; i++) {
                    let $outerdiv = $(
                        document.getElementById(this.inputs[i].inputid)).
                        parents('div.que.stack').
                        first();
                    if ($outerdiv &&
                        ($outerdiv.hasClass('dfexplicitvaildate') ||
                            $outerdiv.hasClass('dfcbmexplicitvaildate'))) {
                        // With instant validation, we don't need the Check
                        // button, so hide it.
                        let button = $outerdiv.find(
                            '.im-controls input.submit').first();
            ...
            buildInputControls(mode) {
                ...
                this.controls = new VisualMath.ControlList(
                    '#' + this.questionid + 'controls_wrapper');
                let controlNames = [];
            ...
            toggleEditor(save) {
                let stackInputDebug = document.getElementById(
                    this.inputs[0].inputid + '_debug');
                let $stackInputDebug = $(stackInputDebug);
                let debugWrapper = $stackInputDebug.closest(
                    '.stack-debug-wrapper');
        ...
        function saveEditorSelection(questionid, editorSelection) {
            ...
            sessionStorage.setItem('editor_selection',
                JSON.stringify(editorSelectionData));
        }
        ...
        return {
            initialize: (questionid, debug, prefix, inputs, editorOptions) => {
                ...
                let editor = new wysiwyg(questionid, debug, prefix, inputs,
                    editorOptions);
            }
        };
